...
import PixelText from "@workspace/ui/components/PixelText/PixelText";
...
export const CpuCard = ({
    ...
    const isXs = size === "xs";
    const isCompact =
        size === "xs" || size === "sm" || size === "xxs" || size === "xxs-wide";
    ...
    useEffect(() => {
        ...
        const observer = new ResizeObserver(([entry]) => {
            ...
            const cols = Math.max(
                1,
                Math.floor(width / (BLOCK_WIDTH + GAP))
            );
            setColumns(cols);
        });
    ...
    const filledColumns = isOffline
        ? 0
        : Math.round((percentage / 100) * columns);
    ...
    if (isXxs) {
        return (
            <UsageCard
                className={cn(
                    "flex h-full flex-col justify-center gap-2 px-4",
                    isOffline && "opacity-60"
                )}
            >
                {/* Header */}
                <div className="flex items-center justify-between">
          <span className="text-md font-medium uppercase text-zinc-400">
            {labels.title}
          </span>
                    <span
                        className={cn(
            ...
            {isOffline
                ? "--"
                : primaryValue ?? <AnimatedNumber value={percentage} suffix="%" />}
          </span>
                </div>
              ...
              {isOffline ? (
                ...
                {showCoreGrid && coreUsage && (
                    <div className="mt-2 flex min-h-0 flex-1 flex-col overflow-hidden">
                        <div className="mb-1 shrink-0 text-[9px] font-medium uppercase text-zinc-500">
                            {labels.coreUsage}
                        </div>
