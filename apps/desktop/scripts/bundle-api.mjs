...
// Copy the blueprint catalog. seedDefaultBlueprintsIfEmpty reads
// *.blueprint.json from STELLAR_BLUEPRINTS_PATH on first launch.
const blueprintsSrc = join(repoRoot, "packages", "blueprints", "converted")
const blueprintsDest = resolve(desktopRoot, "build", "blueprints")
rmSync(blueprintsDest, { recursive: true, force: true })
mkdirSync(blueprintsDest, { recursive: true })
cpSync(blueprintsSrc, blueprintsDest, { recursive: true })
console.log(`copied blueprints from ${blueprintsSrc}`)
