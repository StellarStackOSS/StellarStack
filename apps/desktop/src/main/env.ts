...
export const env = {
  ...
  /** Where the bundled `packages/blueprints/converted/` tree lives. The
   *  API seeds the catalog from every *.blueprint.json under here on
   *  first launch (when the blueprints table is empty). */
  blueprintsPath: isDev
    ? path.resolve(process.cwd(), "..", "..", "packages", "blueprints", "converted")
    : path.join(process.resourcesPath, "blueprints"),
} as const
