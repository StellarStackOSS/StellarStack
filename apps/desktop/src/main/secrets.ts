...
type Secrets = {
   ...
   *  first launch. */
  daemonNodeId: string
  /** Better-auth's session signing secret. Persisted so sessions
   *  survive relaunches. */
  authSecret: string
  /** ID of the Docker instance the user picked in the bootstrap dropdown
   *  (e.g. "colima-default", "docker-desktop"). Null on first launch. */
  dockerInstanceId: string | null
...
export const getDockerInstanceId = (): string | null => read().dockerInstanceId

export const setDockerInstanceId = (id: string | null): void => {
  write({ ...read(), dockerInstanceId: id })
}
