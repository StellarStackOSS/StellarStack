...
export const buildSetupRoute = (params: {
  ...
  return new Hono()
    ...
    .post("/", async (c) => {
      ...
      // Seed the local node + allocation pool. We only do this once when
      // the desktop app is the caller — `STELLAR_DESKTOP_DAEMON_KEY`
      // gates it. Hosted installs add nodes via the admin UI.
      const desktopKey = env.STELLAR_DESKTOP_DAEMON_KEY
      if (desktopKey !== undefined && desktopKey !== "") {
        await seedLocalNode({ db, daemonKey: desktopKey })
      }
...
const seedLocalNode = async (params: {
  ...
  const { db, daemonKey } = params

  // Skip if a node already exists with this name (idempotent — handles
  // the "user wiped users but left node row" edge case).
  const existingByName = await db
    .select({ id: nodesTable.id })
    .from(nodesTable)
    .where(eq(nodesTable.name, "local"))
    .limit(1)
  let nodeId = existingByName[0]?.id
  ...
  if (nodeId === undefined) {
    ...
    // Make sure the key matches what the daemon was started with — if
    // the user reinstalled and electron-store regenerated, sync the DB.
    await db
      .update(nodesTable)
      .set({ daemonPublicKey: daemonKey })
