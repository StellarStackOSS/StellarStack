 ...
 * Contains file config, startup detection, log config, and stop command.
 */
...
/**
 * Daemon file info entry returned from file listing.
 */
...
/**
 * Prisma-compatible server update data shape.
 */
...
/**
 * Prisma where clause for activity log queries.
 */
interface ActivityLogWhere {
  serverId?: string;
  event?: { startsWith: string };
}
...
/**
 * Server record with optional BigInt fields requiring serialization.
 * Prisma returns BigInt for memory/disk/swap which need string conversion for JSON.
 */
interface SerializableServer {
  memory?: bigint | number | null;
  disk?: bigint | number | null;
  swap?: bigint | number | null;
  autoShutdownEnabled?: boolean | null;
  autoShutdownTimeout?: number | null;
 ...
 * @returns Serialized server with BigInt fields as strings
 */
...
const createServerSchema = z.object({
  ...
  config: z.record(z.unknown()).optional(), // Override blueprint config
 ...
 *
 * @param node - Node connection details
 * @param method - HTTP method
 * @param path - API path
 * @param body - Optional request body (will be JSON-serialized)
 * @param options - Response format options
 * @returns Parsed response from daemon
 */
...
servers.post("/", RequireAdmin, async (c) => {
  ...
  let server = await db.server.create({
    data: {
      ...
      variables: variablesWithPort as unknown as Prisma.InputJsonValue,
  ...
  const startupConfig = blueprint.config as unknown as BlueprintConfig;
...
servers.patch("/:serverId", RequireServerAccess, async (c) => {
  ...
  const updated = await db.server.update({
    where: { id: server.id },
    data: parsed.data as unknown as ServerUpdateData,
  });
...
servers.post("/:serverId/change-blueprint", RequireServerAccess, async (c) => {
  ...
  if (parsed.data.reinstall && updated.node.isOnline) {
    try {
      ...
      console.error(`[Change Blueprint] Failed to reinstall server ${server.id}:`, errorMsg);
      await db.server.update({
        where: { id: updated.id },
        data: { status: "ERROR" },
      });
...
servers.get("/:serverId/stats", RequireServerAccess, async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      is_installing: (serverInfo.is_installing as boolean) || false,
      is_transferring: (serverInfo.is_transferring as boolean) || false,
      is_restoring: (serverInfo.is_restoring as boolean) || false,
    });
  } catch (error: unknown) {
    return c.json({ error: error instanceof Error ? error.message : "Unknown error" }, 500);
  }
...
servers.patch("/:serverId/startup", RequireServerAccess, async (c) => {
  ...
  if (parsed.data.variables) {
    ...
    updateData.variables = newVariables as unknown as Prisma.InputJsonValue;
  }
...
servers.get("/:serverId/files", RequireServerAccess, async (c) => {
  ...
  try {
    ...
    const files = (Array.isArray(rawFiles) ? rawFiles : []).map((f: DaemonFileInfo) => ({
      name: f.name,
...
servers.get("/:serverId/backups", RequireServerAccess, async (c) => {
  ...
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : "Unknown error";
    console.error(`[Backup List] Failed to list backups for server ${server.id}:`, errorMsg);
...
servers.post("/:serverId/backups", RequireServerAccess, RequireNotSuspended, async (c) => {
  ...
  try {
    ...
    if (backupResponse.success) {
      ...
      await db.backup.create({
        data: {
          id: backupUuid,
          name: backupName,
          size: (backupResponse.size as number) || 0,
          checksum: backupResponse.checksum as string,
          checksumType: "sha256",
          status: "COMPLETED",
    ...
    const errorMsg = error instanceof Error ? error.message : "Unknown error";
    console.error(`[Backup Create] Failed to create backup:`, errorMsg);
    return c.json({ error: errorMsg }, 500);
  }
});
...
servers.post("/:serverId/backups/restore", RequireServerAccess, RequireNotSuspended, async (c) => {
  ...
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : "Unknown error";
    console.error(`[Backup Restore] Failed to restore backup ${id}:`, errorMsg);
...
servers.delete("/:serverId/backups/delete", RequireServerAccess, RequireNotSuspended, async (c) => {
  ...
  try {
    ...
    try {
      ...
      const daemonErrMsg = daemonError instanceof Error ? daemonError.message : String(daemonError);
      console.warn(`[Backup Delete] Daemon error deleting backup ${id}:`, daemonErrMsg);
      // If daemon file not found, still remove from database (cleanup)
      if (daemonErrMsg.includes("404") || daemonErrMsg.includes("not found")) {
        console.log(
    ...
    const errorMsg = error instanceof Error ? error.message : "Unknown error";
    console.error(`[Backup Delete] Failed to delete backup ${id}:`, errorMsg);
    return c.json({ error: errorMsg }, 500);
  }
});
...
servers.patch("/:serverId/backups/lock", RequireServerAccess, async (c) => {
  ...
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : "Unknown error";
    console.error(`[Backup Lock] Failed to update backup ${id}:`, errorMsg);
    return c.json({ error: errorMsg }, 500);
  }
});
...
servers.post("/:serverId/schedules", RequireServerAccess, async (c) => {
  ...
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : "Unknown error";
    console.error("[Schedule] Failed to create schedule:", errorMsg);
    return c.json({ error: errorMsg }, 500);
  }
});
...
servers.patch("/:serverId/schedules/:scheduleId", RequireServerAccess, async (c) => {
  ...
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : "Unknown error";
    console.error("[Schedule] Failed to update schedule:", errorMsg);
    return c.json({ error: errorMsg }, 500);
  }
});
...
servers.delete("/:serverId/schedules/:scheduleId", RequireServerAccess, async (c) => {
  ...
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : "Unknown error";
    console.error("[Schedule] Failed to delete schedule:", errorMsg);
    return c.json({ error: errorMsg }, 500);
  }
});
...
servers.post("/:serverId/schedules/:scheduleId/run", RequireServerAccess, async (c) => {
  ...
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : "Unknown error";
    console.error("[Schedule] Failed to trigger schedule:", errorMsg);
    return c.json({ error: errorMsg }, 500);
  }
});
...
servers.get("/:serverId/activity", RequireServerAccess, async (c) => {
  ...
  const where: ActivityLogWhere = { serverId: server.id };
  if (event) {
