...
import { pluginWorkerPool } from "./PluginWorker";
import type { PluginWorker } from "./PluginWorker";
...
/** A plugin action definition from the manifest */
...
/** Partial Prisma plugin record fields used by the executor */
      ...
      if (typeof value === "string") {
        resolved[key] = value
          ...
          .replace(/\{\{config\.([^}]+)\}\}/g, (_, configKey) => {
            const configValue = context.config[configKey];
