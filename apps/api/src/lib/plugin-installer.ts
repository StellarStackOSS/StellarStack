...
export class PluginInstaller {
   ...
   */
  async installFromGit(options: InstallOptions): Promise<any> {
   ...
   */
  async update(pluginId: string): Promise<any> {
    ...
    const gitRepoUrl = (plugin as any).gitRepoUrl;
    if (!gitRepoUrl) {
    ...
    return await this.installFromGit({
      repoUrl: gitRepoUrl,
      branch: (plugin as any).gitBranch || "main",
      trustLevel: (plugin as any).trustLevel || "community",
    });
  }
  ...
  private async analyzeSecurityAsync(pluginDir: string): Promise<SecurityReport> {
    const { PluginSecurityAnalyzer } = await import("./plugin-security");
  ...
  private async createPlugin(
    ...
    return await db.plugin.create({
      data: {
        ...
        configSchema: (manifest.configSchema as any) || null,
        trustLevel,
        securityScore: securityReport.score,
        securityReport: securityReport as any,
        gitRepoUrl,
        gitBranch: "main",
