import { db } from "./Db";
import type { WebhookEvent, WebhookPayload, DispatchWebhookOptions } from "./WebhooksTypes";

// Re-export types for backwards compatibility
export type { WebhookEvent, WebhookPayload, DispatchWebhookOptions } from "./WebhooksTypes";
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      await db.webhookDelivery.update({
        where: { id: deliveryId },
        data: {
          response: error instanceof Error ? error.message : "Retry failed",
        },
      });
